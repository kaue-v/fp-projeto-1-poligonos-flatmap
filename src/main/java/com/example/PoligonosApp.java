...
public class PoligonosApp extends Application {
    ...
    protected List<String> tipoPoligonos(){
        return pontosPoligonos.stream()
                ...
                .map(PoligonosApp::nomeTipoPoligono)
                .toList();
    }

    private static String nomeTipoPoligono(final int qtdPontos) {
        return switch (qtdPontos) {
            case 3 -> "Triângulo";
            case 4 -> "Quadrilátero";
            case 5 -> "Pentágono";
            case 6 -> "Hexágono";
    ...
    protected List<Double> perimetros(){
        return pontosPoligonos.stream()
                .flatMap(pontos -> {
                    ...
                    return Stream.of(pontos.stream().reduce(ultimoPonto, Point::new));
                })
                .map(Point::distance)
                .toList();
    }
